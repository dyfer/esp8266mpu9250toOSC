...
void readAllSensors() {
  ...
  MadgwickQuaternionUpdate(-ax, ay, az, gx * PI / 180.0f, -gy * PI / 180.0f, -gz * PI / 180.0f,  my,  -mx, mz, q, deltat); //remember to pass quaternion!
  //MadgwickQuaternionUpdate(-ax, ay, az, gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f,  my,  -mx, mz, q, deltat); //from another code... not tested
  // MahonyQuaternionUpdate(-ax, ay, az, gx*PI/180.0f, -gy*PI/180.0f, -gz*PI/180.0f, my, -mx, mz, q, deltat); //not tested after changing to local vars
...
void calibrateAccelGyroMPU9250(float * localGyroBias, float * localAccBias)
{
  ...
  for (ii = 0; ii < packet_count; ii++) {
    ...
    for (int i = 0; i < 3; i ++ ) {
      Serial.print(accel_temp[i]);
      Serial.print(" ");
    };
    Serial.println("");
    Serial.print("gyro_temp: ");
    for (int i = 0; i < 3; i ++ ) {
      Serial.print(gyro_temp[i]);
      Serial.print(" ");
    };
    Serial.println("");
...
void readRealAccel(OSCMessage &msg) { //is this correct?
  /* if(readDmpFifo()) { */
  /* mpu.dmpGetQuaternion(&q, fifoBuffer); */
...
void readWorldAccel(OSCMessage &msg) { //fixme: how's that related to lin_ax etc?
  /* if(readDmpFifo()) { */
  /* mpu.dmpGetQuaternion(&q, fifoBuffer); */
  /* mpu.dmpGetAccel(&aa, fifoBuffer); */
  /* mpu.dmpGetGravity(&gravity, &q); */
